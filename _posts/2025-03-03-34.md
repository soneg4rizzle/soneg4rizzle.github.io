...
아래와 같이 프리다(Frida) 디폴트 포트의 활성화 여부로 탐지하는 경우에는 해당 함수를 후킹하여 인자 값을 변조하는 방식으로 우회가 가능  
...
`StringsKt.contains$default((CharSequence) it.next(), (CharSequence) "frida", false, 2, (Object) null)` 코드 부분에서 `contains$default` 함수의 2번째 인자("frida")를 후킹하여 값을 변조하거나 "isCheckFridaModule" 함수 자체의 리턴 값을 변조하면 우회가 가능합니다.

현재 제 진단기기에서는 아래와 같이 `frida` 관련 모듈이 탐색되지 않아 특별한 우회 과정은 필요하지 않았습니다.
